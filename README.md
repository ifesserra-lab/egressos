...
Dashboard executivo **anonimizado** da trajetória de carreira e renda de **37 egressos** que passaram pela mesma base na formação: **ensino** (monitoria), **pesquisa aplicada** com bolsa da **FAPES** (via Prodest) e **extensão** no **LEDS/IFES** — e hoje ocupam posições sênior em engenharia de software, dados, consultoria, liderança técnica e gestão de TI (vários no exterior).
...
- **37 de 37** seguem em tecnologia (1 no setor público, em dados).
...
- **21 dos 37** começaram em bolsa/estágio (projeto FAPES ou extensão LEDS/IFES).
- **13 dos 37** atuam em empresas internacionais; **9 dos 37** já lideram ou gerenciam.
...
- `dashboard_alunos.html` — panorama por egresso (A–AK): linha do tempo e cards individuais, anonimizados.
